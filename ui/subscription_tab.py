...
class SubscriptionTab(QWidget):
    def __init__(self, main_window=None):
        ...
        # Developer mode refresh lebih sering untuk testing
        # Import QTimer jika belum diimpor (misal: from PyQt6.QtCore import QTimer)
        interval = 10000 if self.cfg.get("debug_mode", False) else 30000  # 10s dev, 30s normal
    ...
    def set_developer_mode(self, enabled=True):
        """Set mode developer untuk subscription tab."""
        if enabled:
            # Add developer status indicator
            if hasattr(self, 'credit_label'):
                self.credit_label.setText("Kredit: ∞ jam (Developer Mode)")
                self.credit_label.setStyleSheet("color: #4CAF50; font-weight: bold;")

            if hasattr(self, 'usage_label'):
                self.usage_label.setText("Developer Mode Active - Unlimited Access")

            if hasattr(self, 'info_label'):
                self.info_label.setText("✅ Developer Mode - All Features Unlocked")
                self.info_label.setStyleSheet("color: #4CAF50; font-weight: bold;")

            # Enable access button
            if hasattr(self, 'access_btn'):
                self.access_btn.setText("🚀 Akses Developer Mode")
                self.access_btn.setEnabled(True)
    ...
    def refresh_credit_info(self, silent=False):
        ...
        try:
            # TAMBAHAN: Skip untuk test mode (tapi bukan dev mode!)
            if hasattr(self.main_window, 'license_validator') and self.main_window.license_validator.testing_mode and not self.cfg.get("debug_mode", False):
            ...
            # Skip untuk debug mode (tapi tetap baca file subscription untuk testing)
            if self.cfg.get("debug_mode", False):
                # Tetap baca subscription_status.json untuk developer mode
                ...
                from PyQt6.QtWidgets import QMessageBox # Pastikan QMessageBox diimpor
                ...
                if subscription_file.exists():
                    try:
                        ...
                        # Ambil data dari file, tapi dengan fallback unlimited
                        hours_credit = float(subscription_data.get("hours_credit", 999))
                        hours_used = float(subscription_data.get("hours_used", 0))
                        package_name = subscription_data.get("package", "basic")
                        status = subscription_data.get("status", "paid")

                        # Update display dengan data real dari file
                        self.credit_label.setText(f"Kredit: {hours_credit:.1f} jam (Dev Mode)")
                        self.usage_label.setText(f"Pemakaian: {hours_used:.2f} jam (Developer)")
                        ...
                        if hours_credit + hours_used > 0:
                            ...
                            self.credit_bar.setFormat(f"{int(usage_percent)}% terpakai (Dev)")
                        else:
                            self.credit_bar.setValue(0)
                            self.credit_bar.setFormat("0% (Dev)")

                        # Info status
                        if status == "paid" and hours_credit > 0:
                            self.info_label.setText(f"✅ Developer Mode - {package_name.capitalize()} Active")
                            self.info_label.setStyleSheet("color: #4CAF50; font-weight: bold;")

                            # Enable access button
                            if hasattr(self, 'access_btn'):
                                self.access_btn.setText(f"🚀 Akses {package_name.capitalize()} (Dev)")
                                self.access_btn.setEnabled(True)
                                self.access_btn.setStyleSheet("""
                                    font-size: 16px; padding: 10px 20px;
                                    background-color: #4CAF50; color: white;
                            ...
                            self.info_label.setText("⚠️ Developer Mode - No Subscription Data")
                            self.info_label.setStyleSheet("color: #FF9800; font-weight: bold;")

                        # Update session status
                        self.session_status.setText("🔧 Mode Developer - Data dari file subscription")
                        self.session_status.setStyleSheet("color: #1877F2;")

                        # PERBAIKAN: Tampilkan notifikasi jika ada pembelian baru
                        # Perlu dipastikan 'silent' didefinisikan atau dijadikan parameter default True
                        silent = True # Asumsi default silent adalah True jika tidak diberikan
                                ...
                                if (now - updated_at).total_seconds() < 300:  # 5 menit terakhir
                                    if not silent:
                                        QMessageBox.information(
                                            self, "Refresh Berhasil",
                                            f"Kredit berhasil diperbarui!\n"
                                            f"Paket: {package_name.capitalize()}\n"
                                            f"Kredit: {hours_credit:.1f} jam"
                                        )
                                    self.info_label.setText(self.info_label.text() + " | ✨ Pembelian baru!")
                            except:
                                pass # Abaikan error parsing tanggal

                        return # Keluar dari fungsi jika debug mode aktif dan file ada

                    except Exception as e:
                        print(f"[DEBUG] Error reading subscription in dev mode: {e}")

                # Fallback ke unlimited jika file tidak ada atau ada error baca file
                self.set_developer_mode(True)
                self.credit_label.setText("Kredit: ∞ jam (Dev Mode - No File)")
                ...
                self.session_status.setStyleSheet("color: #808080;")
                return # Keluar dari fungsi setelah set developer mode
            ...
            if subscription_file.exists():
                try:
                    ...
                    # PERBAIKAN: Baca langsung dari file tanpa filter status untuk dev mode
                    if self.cfg.get("debug_mode", False):
                        # Dev mode - baca langsung dari file
                        hours_credit = float(subscription_data.get("hours_credit", 0))
                        hours_used = float(subscription_data.get("hours_used", 0))
                    else:
                        # Production mode - filter berdasarkan status
                    ...
                    # Update display
                    self.credit_label.setText(f"Kredit: {hours_credit:.1f} jam")
    ...
    def force_refresh_credit(self):
        ...
        # PERBAIKAN: Untuk developer mode, selalu baca file terbaru
        if self.cfg.get("debug_mode", False):
            print("[DEBUG] Force refresh in developer mode")
            self.refresh_credit_info(silent=True)
            return True
        """Paksa refresh kredit dari file, bukan dari cache."""
        try:
            subscription_file = Path("config/subscription_status.json")
    ...
    def access_package(self):
        ...
        # PERBAIKAN: Untuk developer mode, selalu izinkan akses
        if self.cfg.get("debug_mode", False):
            subscription_file = Path("config/subscription_status.json")
            if subscription_file.exists():
                try:
                    ...
                    if hasattr(self.main_window, 'pilih_paket'):
                        ...
                        QMessageBox.information(
                            self, "Aktivasi Berhasil (Dev Mode)",
                            f"Paket {package.upper()} telah diaktifkan dalam mode developer!"
                        )
                        return True
                except Exception as e:
                    print(f"[DEBUG] Error accessing package in dev mode: {e}")

            # Fallback untuk dev mode jika file tidak ada atau ada error
            package = "basic"
            self.cfg.set("paket", package)
            if hasattr(self.main_window, 'pilih_paket'):
                self.main_window.pilih_paket(package)
    ...
    def buy_credit(self, hours, price):
        ...
        # PERBAIKAN: Dapatkan email dengan fallback untuk developer mode
        email = self.cfg.get("user_data", {}).get("email")
        
        # Fallback untuk developer mode
        if not email and os.getenv("STREAMMATE_DEV", "").lower() == "true":
            email = "dev@streammate.ai"
            self.cfg.set("user_data", {"email": email, "name": "Developer User"})
            print(f"[DEBUG] Developer mode: using fallback email {email}")
    ...
    def start_demo(self):
        """Mulai mode demo dengan validasi server yang diperbaiki."""
        try:
            from datetime import datetime, timedelta
            import time
            from pathlib import Path
            ...
            # PERBAIKAN: Cek apakah demo sudah aktif
            subscription_file = Path("config/subscription_status.json")
            ...
            if subscription_file.exists():
                try:
                    ...
                    if data.get("status") == "demo":
                        ...
                        if expire_date_str:
                            demo_expire = datetime.fromisoformat(expire_date_str)
                            if demo_expire > datetime.now():
                                demo_already_active = True
                                remaining = demo_expire - datetime.now()
                                mins_left = int(remaining.total_seconds() / 60)

                                # Jika demo masih aktif, langsung akses tanpa reset waktu
                                reply = QMessageBox.question(
                                    self,
                                    "Demo Masih Aktif",
                                    f"Demo masih aktif dengan sisa {mins_left} menit.\n\n"
                                    "Lanjutkan ke mode Basic?",
                                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
                                )

                                if reply == QMessageBox.StandardButton.Yes:
                                    # Langsung akses tanpa register ulang
                                    self.cfg.set("paket", "basic")
                                    # Pastikan refresh_credit_info dipanggil
                                    self.refresh_credit_info(True)
                    ...
                    print(f"[DEBUG] Error checking existing demo: {e}")

            # Dapatkan email user (kode existing tetap sama)
            email = self.cfg.get("user_data", {}).get("email", "")
            if not email:
                QMessageBox.warning(
                    self, "Error",
            ...
            # Cek demo availability dari server (kode existing tetap sama)
            import requests
            try:
                response = requests.post(
            ...
            # PERBAIKAN: Konfirmasi demo hanya untuk demo baru
            if not demo_already_active:
                reply = QMessageBox.question(
                    self,
                    "Konfirmasi Demo",
                    "Mode demo akan aktif selama 30 menit.\n"
                    "Waktu akan terus berjalan meskipun berpindah menu.\n\n"
                    "Lanjutkan?",
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
                )
            else:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                # Jika ini adalah demo baru, lakukan pendaftaran
                if not demo_already_active:
                ...
                # Set subscription status untuk demo
                subscription_data = {
                    "email": email,
                    "package": "basic",
                    "status": "demo",
                    ...
                    "expire_date": demo_expire.isoformat(), # Menggunakan demo_expire yang sudah valid
                ...
                # Pilih paket melalui main_window
                if hasattr(self, 'main_window') and self.main_window is not None:
                    if hasattr(self.main_window, 'pilih_paket'):
                        self.main_window.pilih_paket("basic")
                    else:
            ...
            from datetime import datetime # Pastikan datetime diimpor untuk error handling
            QMessageBox.critical(
                self, "Error",
            ...
            print(f"[ERROR] Error in start_demo: {e}")
            # Import datetime untuk error handling
