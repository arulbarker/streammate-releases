...
class ProfileTab(QWidget):
    ...
    def buy_credit(self, hours=100):
        ...
        # PERBAIKAN: Navigasi yang lebih robust
        ...
        # Metode 1: Cek jika ada stack widget dan subscription_tab
        if hasattr(self.parent_window, 'stack') and hasattr(self.parent_window, 'subscription_tab'):
            if self.parent_window.subscription_tab is not None:
                try:
                    self.parent_window.stack.setCurrentWidget(self.parent_window.subscription_tab)
                    QMessageBox.information(
                        self, 
                        "Navigasi Berhasil", 
                        f"Silakan beli {hours} jam kredit di tab Subscription."
                    )
                    return
                except Exception as e:
                    print(f"[DEBUG] Error navigating to existing subscription tab: {e}")
        
        # Metode 2: Buat subscription tab baru jika belum ada
        try:
            ...
            if not hasattr(self.parent_window, 'subscription_tab') or self.parent_window.subscription_tab is None:
                print(f"[DEBUG] Creating new subscription tab")
            ...
            # Set sebagai current widget
            if hasattr(self.parent_window, 'stack'):
                self.parent_window.stack.setCurrentWidget(self.parent_window.subscription_tab)
                QMessageBox.information(
                    self, 
                    "Navigasi Berhasil", 
                    f"Tab Subscription dibuka. Silakan beli {hours} jam kredit."
                )
                return
        except Exception as e:
            print(f"[DEBUG] Error creating/navigating to subscription tab: {e}")
        
        # Fallback: Tampilkan pesan jika semua metode gagal
        QMessageBox.information(
            self, 
            "Beli Kredit Jam", 
            f"Silakan buka tab Subscription secara manual untuk membeli {hours} jam kredit.\n\n"
            f"Jika dalam mode developer, gunakan tombol 'Subscription Tab' di menu utama."
        )

    def to_subscription(self):
        """Navigasi langsung ke tab Subscription dengan metode yang lebih robust."""
        # PERBAIKAN: Logika navigasi yang lebih sederhana dan robust
        if not self.parent_window:
            return
        
        try:
            # Jika dalam developer mode, buka subscription tab individual
            if hasattr(self.parent_window, 'open_single_tab'):
                self.parent_window.open_single_tab("subscription")
                return
            
            # Metode normal: gunakan buy_credit untuk navigasi
            self.buy_credit()
            
        except Exception as e:
            print(f"[DEBUG] Error in to_subscription: {e}")
            QMessageBox.information(
                self, 
                "Navigasi", 
                "Silakan buka tab Subscription secara manual."
            )
